...
Alternatively, FreeSASA can be built using CMake:
...
CMake options (passed as `-D<option>=ON/OFF`):

| Option | Default | Description |
|--------|---------|-------------|
| `ENABLE_JSON` | OFF | JSON output support (requires json-c) |
| `ENABLE_XML` | OFF | XML output support (requires libxml2) |
| `ENABLE_THREADS` | ON | Pthread support for multithreaded calculations |
| `ENABLE_TESTS` | ON | Unit and CLI tests (requires [Check](https://libcheck.github.io/check/)) |
| `ENABLE_DOXYGEN` | OFF | Build API documentation with Doxygen |
| `ENABLE_PARSER_GENERATOR` | OFF | Regenerate lexer/parser with Flex and Bison |
| `ENABLE_GCOV` | OFF | Coverage instrumentation (gcov) |

Example with all features enabled:

    cmake -B build -DENABLE_JSON=ON -DENABLE_XML=ON -DENABLE_TESTS=ON
    cmake --build build
