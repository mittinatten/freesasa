...
/** This set of functions maps between different classes of atoms and
    residues. In addition, the function sasalib_classify_radius(2)
    ...
    value of n for that class, e.g. sasalib_classify_nclasses() or
    sasalib_classify_nelements(). Each class also has a function that
    converts the integer to a descriptive string,
    e.g. sasalib_classify_class2str(1).
...
/** Given an atom this function returns an atomic radius. Regular
    ...
    sasalib_classify_element_radius()). Unknown atom types and
...
/** Returns an integer signifying the element of an atom. Return
    values span from 0 to sasalib_classify_nelements()-1. */
...
/** Returns an integer signifying the type of an atom according to
    ...
    sasalib_classify_noons(). Hydrogen and Selenium atoms give return
    value sasalib_oons_unknown */
int sasalib_classify_oons(const char *res_name, const char *atom_name);
...
/** Returns a class (polar/apolar/etc) for an OONS type. Will not
    return the sasalib_classify_nucleic, since this in not part of the
    OONS scheme. */
