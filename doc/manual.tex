...
The API is designed to cater to the three use cases outlined in the
introduction. Any calculation involves the following four steps.
\begin{enumerate}
...
used separately if users wish to handle the other steps themselves. In
addition to the description below, the source for \texttt{calc\_sasa.c}
serves as an example of how to use the library, using a large portion
...
etc. Users with specific needs can also use the functions in the other
headers, but these will not be documented beyond comments in the
header-files.

\subsection{Error-handling}

When an error is detected it will be reported to STDERR as a error
...
used are \texttt{SASALIB\_SUCCESS}, \texttt{SASALIB\_WARN} and
\texttt{SASALIB\_FAIL}. The first means no errors were detected; the
...
Sasalib defines one struct, \texttt{sasalib\_t}, that stores both
parameters, results and can be used to link coordinates stored
elsewhere for repeated calculations. This struct is initalized using
\begin{verbatim}
...
\end{verbatim}
The object generated by \texttt{sasalib\_init()} has default
parameters, and can be used directly to perform a calculation, given the
correct input.
...
return \texttt{SASALIB\_WARN}, and proceed with the previously defined
...
\texttt{SASALIB\_LEE\_RICHARDS} and \texttt{SASALIB\_SHRAKE\_RUPLEY}.
The third function below returns a string representation of the
...
\texttt{sasalib\_t}-objects will have an assigned algorithm.
...
\end{verbatim}
Finally, parameters can be copied between to \texttt{sasalib\_t}
objects using
\begin{verbatim}
...
\end{verbatim}
Protein names can be no longer than \texttt{SASALIB\_NAME\_LIMIT}
...
\subsection{The other headers}

The following will give a brief description of the other headers in
Sasalib. These are primarily for internal use within the library, and
will therefore only be documented through comments in the headers
themselves. This listing is meant to give a brief introduction for
users who wish to understand the implementation better.

\begin{tabular}{r>{\raggedright\arraybackslash}p{0.7\textwidth}}
\texttt{structure.h} & Functions to deal with PDB structures, used 
                       to organize coordinates and other information
