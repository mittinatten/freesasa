...
parallelized in Sasalib. As mentioned above, in S\&R each atom can be treated
...
\verb|sasalib_t|-object can be initalized using
\begin{verbatim}
...
\verb|sasalib_generate_radii(2)| described above) and then these can
be used as input for \verb|sasalib_link_coord(4)|. Remember that
hydrogen atoms are \emph{not} read from PDB files by Sasalib, so special
