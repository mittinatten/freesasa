...
\begin{figure}
%\begin{center}
\includegraphics{fig/lnr_slice.eps}
\includegraphics{fig/lnr_circles.eps}
...
The L\&R SASA calculation in Sasalib begins by finding out which of
the extended atoms are in contact. These contacts are stored in an
adjacency list. Then, for each slice the adjacency list is condensed
to a separate list, only containing the atoms in that slice. Each pair
of atoms in the reduced list is then checked for overlap in the
particular slice and the overlapping arcs are calculated as described
in section \ref{sec:alg_LnR}. When all buried arcs have been counted
for a given atom they are reduced to non-overlapping intervals
recursively. In most cases only one recursion step is necessary. The
arc-lengths are then summed up for all atoms in the slice to calculate
the total contribution to the area as described in \ref{sec:alg_LnR}.
