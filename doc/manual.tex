...
measure the surface that is buried in protein oligomers. To
define the SASA, $A$, of a given molecule, let a spherical probe roll
over the surface of the molecule, including internal cavities. $A$ is
then the surface drawn by the center of the probe.~\cite{LnR} The
probe represents a solvent molecule (i.e.\ water). Cavities smaller
than the solvent molecule do not contribute to $A$.

Calculating SASA is a run-of-the-mill calculation in protein structure
studies. The probably most commonly used tool is the program
NACCESS~\cite{NACCESS}, which is a Fortran implementation of the
approximation algorithm by Lee \& Richards~\cite{LnR}, freely
available to academic users. Some simulation packages include tools to
calculate SASA. Gromacs for example has an efficient implementation of
the algorithm by Shrake \& Rupley~\cite{SnR,DCLM}. In addition, there
is at least one web server available, using an exact analytic
calculation~\cite{Getarea}. But, to the author's knowledge there are
no fully open source, standalone programs, nor any libraries designed
to be easily integrated into other programs. The present library is an
attempt to resolve both these issues, and is released under the GNU
General Public License 3 to facilitate reuse.
...
straightforward calculations (using the atomic radii from Ooi et
al.~\cite{OONS}), but also allows the user to change most parameters
of the calculation. In addition, the user can treat the calculation as
a purely mathematical operation on a set of spheres, by providing
arbitrary coordinates and atomic radii. Both Lee \& Richards'
\cite{LnR} and Shrake \& Rupley's \cite{SnR} algorithms have been
implemented. They will be referred to as L\&R and S\&R throughout this
document. In the current implementation S\&R is the more effecient,
and therefore recommended for most applications.
...
This document is organized as follows. Section \ref{sec:installation}
describes how to install the program, section \ref{sec:cli} how to
used the CLI, and section \ref{sec:api} gives a short sample program
demonstrating basic use of the API. The introduction is intended to
cover most casual users' needs. Section \ref{sec:performance} analyzes
the performance of the program. Appendix \ref{sec:algorithm} describes
the algorithms, and appendix \ref{sec:implementation} their
implementation. A full Doxygen-generated documentation of the API is
available at \url{http://mittinatten.github.io/freesasa/}, and is also
...
should therefore be straightforward to compile and install. It has
been tested on Mac OS X and a few Linux platforms. Building the git
clone requires autotools, whereas the tarball includes the
autogenerated scripts needed to build and install the program.
...
largest structure studied having over 30\,000 atoms (1JZ8).
...
$0.001\,\mathrm{\AA}$. The error of a given SASA-value, $S$ is then $\epsilon =
\lvert S - S_\text{ref} \rvert / N$, where $N$ is the number of atoms
in the protein. Figure~\ref{fig:precision} shows the results of these
...
present implementation. The value $0.25\,\mathrm{\AA}$ for example is close to
the default accuracy of NACCESS\footnote{The parameter $z$ in NACCESS
  is multiplied by the atomic diameter to get the slice width. The
  default $z=0.05$ thus gives a slice width of $0.28-0.32\,\mathrm{\AA}$
  depending on the atom type.}~\cite{NACCESS}, but for the same
computational effort an order of magnitude better accuracy can be
achieved using the FreeSASA implementation of S\&R with 500
test-points.
...
FreeSASA is an open-source stand-alone program for calculating
SASA. It can be used both as command-line tool and as an API. The
implementation of Lee and Richards' algorithm has similar computation
time to NACCESS, and the Shrake and Rupley implementation is about an
order of magnitude faster for the same level of precision.
...
with adjacent atoms in each slice. The buried arcs due to overlapping
circles are calculated as described in section \ref{sec:alg_LnR}. If
the atom is not completely buried, the exposed arc length is
calculated and it's contribution to the total area calculated
using~\ref{eq:LR_SASA}.
