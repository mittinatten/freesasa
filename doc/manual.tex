...
etc. It is the main interface to library, and is the only
header installed by \verb|make install|. The following will outline
some of the most important functions in this header, a full
documentation can be built from source using Doxygen. In addition, the
source for \texttt{main.c} serves as an illustration of how to use the
library, using a large portion of the available functionality. 
...
each time the residue number changes, it is assumed to correspond to a
new residue. Users who need more advanced and/or robust handling of
PDB files are recommended to use external libraries for this.
...
%% also to assign radii to the individual atoms. 
...
to supply these as well in both the above cases.  When new SASA values
are needed these can be recalculated using
\begin{verbatim} 
