...
START_TEST (test_coord)
{
    ...
    ck_assert(fabs(sasalib_coord_dist2(coord,0,3)-6) < 1e-10);
    
    sasalib_coord_set_all_xyz(coord,x,y,z,2);
    ck_assert(sasalib_coord_n(coord) == 2);
    ck_assert(fabs(sasalib_coord_dist2(coord,0,1)-2) < 1e-10);

    sasalib_coord_set_length_i(coord,1,6);
    const double *ci = sasalib_coord_i(coord,1);
    ck_assert(ci != NULL);
    ck_assert(ci[0] == 4 && ci[1] == 4 && ci[2] == 2); 

    sasalib_coord_set_length_all(coord,3);
    ck_assert(ci[0] == 2 && ci[1] == 2 && ci[2] == 1);

    sasalib_coord_t *c2 = sasalib_coord_new();
    ck_assert(c2 != NULL);
    sasalib_coord_append(c2,xyz2,1);
    ck_assert(fabs(sasalib_coord_dist2_12(coord,c2,0,0) - 2));
    sasalib_coord_free(c2);
}
END_TEST
