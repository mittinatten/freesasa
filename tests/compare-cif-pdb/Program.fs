...
type CalculationError = CalculationFailed of Output | DownloadFailed of String
type FreesasaResult = Result<Output, CalculationError>
type ComparisonError = CalculationError of CalculationError list | CalculationDifference
...
let downloadPdb code format =
    let getUrl (PdbCode code) = Uri(archiveUrl, code + (getExtension format))
    let getPath (PdbCode code) = dataDirectory + code + (getExtension format)
    ...
    async {
        ...
        return
            match result with
            | Ok () -> Ok { Path = path; Format = format; Code = code }
...
let normalizeCommandResult (commandResult: CommandResult): Output =
...

    {
        ...
        Warnings = commandResult.StandardError
...
let runPdbCalc args file  =
    ...
    let allArgs = if file.Format = CIF then "--cif" :: args else args
    async {
        let! output = executeCommand freesasaProgram (file.Path :: allArgs) |> Async.map normalizeCommandResult
        do log output
        return
            match output with
...
let calcStructure code fileType args =
    downloadPdb code fileType |> AsyncResult.bind (runPdbCalc args)
...
let compareResults: ResultComparer =
    fun code result1 result2 ->
        ...
        | Error e1, Error e2 ->
            if (e1 = e2) then None else Some (code, err)
        | _ -> Some (code, err)

let checkArgs codes args =
    printf "Checking with args %A\n" args
    let computations = codes |> List.map (compareFormats args compareResults)
...
let getCodes fileName =
    System.IO.File.ReadAllLines(fileName) |> Seq.map (String.trimEnd " " >> PdbCode) |> Seq.toList
...
let main argv =
    if argv.Length < 1 then
        ...
        |> List.choose id
        |> fun errList ->
            if List.isEmpty errList then 0
            else 1
